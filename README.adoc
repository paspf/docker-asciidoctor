...
:WAVEDROM_CLI_VERSION: 3.1.1
= Asciidoctor Docker Container
:source-highlighter: coderay

This is a fork of the original https://github.com/asciidoctor/docker-asciidoctor[Asciidocker Docker Container] with all packages required to create Wavedrom timing diagrams.
...
* https://github.com/wavedrom/cli[Wavedrom-cli] {WAVEDROM_CLI_VERSION} (NEW)
* Wavedrom (NEW)
...
Clone repository, enter it and build container with the desired tag. In this case `asciidoctor-wd`:

[source,bash]
----
docker build -t asciidoctor-wd .
----

Remember the tag and the hash of your container, which is displayed after its creation.

Run container and mount the `tests` directory:

[source,bash]
----
docker run -v ./tests:/documents/ -it asciidoctor-wd
----

Build a PDF out of an asciidoc file:
----
cd fixtures
asciidoctor -r asciidoctor-diagram -r asciidoctor-pdf -b pdf sample-with-wavedrom.adoc
----
